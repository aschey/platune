...
const Row: React.FC<RowProps> = ({ path, drivesUsed, mappings, setMappings, width, isWindows, index }) => {
  ...
  const driveSelect = (
    <DriveSelect
      filterable={false}
      items={choices}
      itemRenderer={(item: string, { handleClick }) => (
        <MenuItem
          key={item}
          disabled={drivesUsed.includes(item) && selectedRow !== item && item !== NONE}
          onClick={handleClick}
          text={item}
        />
      )}
...
export const PathMapping: React.FC<PathMappingProps> = ({
  mappings,
  setMappings,
  setOriginalMappings,
  width,
  ...
  const onSaveClick = async () => {
    await putJson<{}>(
      '/updatePathMappings',
      mappings.map((m): NtfsMapping => ({ dir: m.dir, drive: m.drive === NONE ? '' : m.drive }))
    );
    toastSuccess();
      ...
      <div style={{ width: panelWidth, paddingTop: 10 }}>
        {noMappings ? (
          <div style={{ paddingLeft: 5 }}>
            <Text>No mappings available</Text>
          </div>
        ) : (
          mappings.map((r, i) => (
            <Row
