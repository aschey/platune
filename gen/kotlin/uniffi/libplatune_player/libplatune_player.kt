...
open class RustBuffer : Structure() {
    ...
    companion object {
        ...
        internal fun free(buf: RustBuffer.ByValue) = rustCall() { status ->
            _UniFFILib.INSTANCE.ffi_libplatune_player_rustbuffer_free(buf, status)
        }
    }
...
private fun uniffiCheckContractApiVersion(lib: _UniFFILib) {
    ...
    val scaffolding_contract_version = lib.ffi_libplatune_player_uniffi_contract_version()
...
class EventSubscription(
    ...
    override suspend fun `recv`(): PlayerEvent {
        ...
        // it's invoked
        var callbackHolder: UniFfiFutureCallbackHandlerTypePlayerEvent_TypePlayerError? = null
        return coroutineScope {
            val scope = this
            return@coroutineScope suspendCoroutine { continuation ->
                try {
                    val callback = UniFfiFutureCallbackHandlerTypePlayerEvent_TypePlayerError(continuation)
...
class PlatunePlayer(
    ...
    override suspend fun `getCurrentStatus`(): PlayerStatus {
        ...
        // Make sure to retain a reference to the callback handler to ensure that it's not GCed before
        // it's invoked
        var callbackHolder: UniFfiFutureCallbackHandlerTypePlayerStatus_TypePlayerError? = null
        return coroutineScope {
            val scope = this
            return@coroutineScope suspendCoroutine { continuation ->
                try {
                    val callback = UniFfiFutureCallbackHandlerTypePlayerStatus_TypePlayerError(continuation)
                    callbackHolder = callback
                    callWithPointer { thisPtr ->
                        rustCall { status ->
...
public object FfiConverterTypePlayerStatus : FfiConverterRustBuffer<PlayerStatus> {
    override fun read(buf: ByteBuffer): PlayerStatus {
        return PlayerStatus(
            ...
            FfiConverterOptionalTypeCurrentPosition.read(buf),
        )
    }
...
public object FfiConverterTypeAudioStatus : FfiConverterRustBuffer<AudioStatus> {
    ...
    override fun allocationSize(value: AudioStatus) = 4

    override fun write(value: AudioStatus, buf: ByteBuffer) {
    ...
    override fun write(value: PlayerEvent, buf: ByteBuffer) {
        when (value) {
            ...
            is PlayerEvent.Seek -> {
                buf.putInt(10)
                FfiConverterTypePlayerState.write(value.`state`, buf)
                FfiConverterDuration.write(value.`time`, buf)
                Unit
            }
            is PlayerEvent.QueueEnded -> {
                buf.putInt(11)
            ...
            is PlayerEvent.Position -> {
                buf.putInt(12)
                FfiConverterTypeCurrentPosition.write(value.`currentPosition`, buf)
                Unit
            }
        }.let { /* this makes the `when` an expression, which ensures it is exhaustive */ }
    }
...
public object FfiConverterOptionalTypeCurrentPosition : FfiConverterRustBuffer<CurrentPosition?> {
    override fun read(buf: ByteBuffer): CurrentPosition? {
        ...
        return FfiConverterTypeCurrentPosition.read(buf)
    }

    override fun allocationSize(value: CurrentPosition?): Int {
        if (value == null) {
            return 1
        } else {
            return 1 + FfiConverterTypeCurrentPosition.allocationSize(value)
        }
    }
    ...
    override fun write(value: CurrentPosition?, buf: ByteBuffer) {
        if (value == null) {
            ...
            buf.put(1)
            FfiConverterTypeCurrentPosition.write(value, buf)
        }
    }
}
...
internal class UniFfiFutureCallbackHandlerTypePlayerStatus_TypePlayerError(val continuation: Continuation<PlayerStatus>) :
    ...
    override fun invoke(_callbackData: USize, returnValue: RustBuffer.ByValue?, callStatus: RustCallStatus.ByValue) {
        try {
            checkCallStatus(PlayerException, callStatus)
            continuation.resume(FfiConverterTypePlayerStatus.lift(returnValue!!))
        } catch (e: Throwable) {
            continuation.resumeWithException(e)
        }
    }
